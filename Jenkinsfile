pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                checkout scmGit(branches: [[name: '*/master']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/mcollins1010/nextcloud-application-docker-pipeline.git']])
            }
        }
        ...
        stage ("Build Image") {
            steps {
                script {
                    // aensure docker user has permission
